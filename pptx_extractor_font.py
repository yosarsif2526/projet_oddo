                ...
                if cell.text.strip():
                    for paragraph in cell.text_frame.paragraphs:
                        for run in paragraph.runs:
                            size_pt = run.font.size.pt if run.font.size else "Default"
                            is_bold = run.font.bold
                            texts.append((y, x, semantic, run.text.strip(), size_pt, is_bold))
...
def extract_pptx_to_txt(pptx_path, output_txt_path):
    ...
    for slide_idx, slide in enumerate(prs.slides, start=1):
        ...
        for y, x, semantic, text, size_pt, is_bold in all_texts_sorted:
            semantic_dict[semantic].append((y, x, text, size_pt, is_bold))

        # Print each semantic once with all texts in y->x order
        for semantic, items in semantic_dict.items():
            lines.append(f"[{semantic}]:\n")
            for _, _, text, size_pt, is_bold in sorted(items, key=lambda t: (t[0], t[1])):
                lines.append(f"{text} (size: {size_pt}, bold: {is_bold})\n")
...
if __name__ == "__main__":
    ...
    output_txt = os.path.join(folder, "slides_extracted_font.txt")
