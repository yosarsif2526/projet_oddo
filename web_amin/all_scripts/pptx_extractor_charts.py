...
def extract_pptx_to_txt(pptx_path, output_txt_path):
    """Extract content from PPTX and save to TXT file."""
    ...
    print(f"Extracted {len(prs.slides)} slides")
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        ...
        print("\nUsage:")
        print(f"  python {sys.argv[0]} <path_to_pptx_file>")
        print("\nExample:")
        print(f"  python {sys.argv[0]} presentation.pptx")
        sys.exit(1)
    ...
    if not pptx_path.lower().endswith('.pptx'):
        print(f"Error: File must be a .pptx file: {pptx_path}")
        sys.exit(1)
    
    file_basename = os.path.basename(pptx_path)
    if file_basename.startswith('~'):
        print(f"Warning: Skipping temporary file: {pptx_path}")
        sys.exit(1)
    
    print("\n" + "="*60)
    print("PPTX EXTRACTION STARTED")
    print("="*60)
    print(f"Input file: {pptx_path}")
    
    pptx_dir = os.path.dirname(pptx_path) or "."
    output_txt_path = os.path.join(pptx_dir, "outputs/slides_extracted_charts.txt")
    
    print(f"Output file: {output_txt_path}")
    print("="*60)
    
    try:
        extract_pptx_to_txt(pptx_path, output_txt_path)
        print("="*60)
